...
def plot_data(df = df, variables = train_cols, days = 60):
    ...
    for var in variables:
        plot2axis(x = pd.to_datetime(dates), y1 = prices, y_name = "WTI Price (USD per Barrel)", color1 = "black", lineax1 = True, lineax1y = df["20dSMA"][-days:], lineax1name = "20 Day SMA", axis2 = True, y2 = df[var][-days:], y2_name = var, fill_boll = True, bol_low = lowBand, bol_high = upBand, bol_name = bollName)


        plt.title('WTI Price & '+var, size = 36)
        plt.tight_layout()
        plt.savefig(os.path.join(OUTPUT_PATH+"featurePlots/", var+'.png'), dpi = 300, bbox_inches = "tight")
            
            
            
focus_features = ['Prices', 'OilProduction', 'NatGasPrices', 'BrentPrices', 'SP500', 'Nasdaq', 'DowJones', 'Gold', 'Bond10y','10dSMA', '20dSMA', '200dSMA', 'daysAbove200dSMA', 'distFromLowBoll', '20d200dDist', 'daysAbove20dSMA', 'distFromTopBoll', 'RSI_14', 'Momentum_14', 'MACD_12_26', 'month']
nonShiftFeat = ['Prices','month']

def plot_correlations(df=df, features=focus_features, nonShiftFeats=nonShiftFeat ):
    sns.set(style="white")
    dfCorr = copy.copy(df)

    focus_cols=create_features(dfCorr, features = focus_features, shift = 1, nonShiftFeatures = nonShiftFeat)
    ...
    heat_fig.suptitle('Correlation between main focus variables', fontsize=20, fontweight='bold')

    plt.savefig(os.path.join(OUTPUT_PATH,'mainFeatureCorrelations.png'), dpi=300, format='png');
    print(corr)
    return corr
...
def create_model():
    ...
    lstm_model.add(LSTM(128, batch_input_shape=(BATCH_SIZE, TIME_STEPS, x_t.shape[2]),
                        dropout=0.0, recurrent_dropout=0.0, stateful=False, return_sequences=True,
...
def train_model(df_train, val_split = 0.2):
...
if model is None or is_update_model:
    model, history, weights = train_model(create_model(), train_data)
...
def test_train_loss(history):
    ...
    plt.savefig(
        os.path.join(
            OUTPUT_PATH, "train_vis_BS_" + str(BATCH_SIZE) + "_" + time.ctime() + ".png"
        )
    )

def visualise_prediction(pred, data):
    plt.figure()
    plt.plot(pred)
    plt.plot(data)
    ...
    plt.savefig(
        os.path.join(
            OUTPUT_PATH,
            "pred_vs_real_BS" + str(BATCH_SIZE) + "_" + time.ctime() + ".png",
        )
    )
...
target_idx = 0
y_pred_org = (y_pred * min_max_scaler.data_range_[target_idx]) + min_max_scaler.data_min_[target_idx]
y_test_t_org = (y_test_t * min_max_scaler.data_range_[target_idx]) + min_max_scaler.data_min_[target_idx]

test_train_loss(history)
visualise_prediction(y_pred_org[-30:], y_test_t_org[-30:])

def predict_new(weights, sc, df, days=1):
    if len(df) < TIME_STEPS:
        raise Exception(
            "To make a prediciton for any following day, {} previous datapoints are required".format(TIME_STEPS)
        )
    new_model = create_model()
    new_model.set_weights(weights)
    data_for_pred = df[train_cols][-TIME_STEPS:]
    x_for_pred = sc.transform(new.loc[:,train_cols])
    
    pred = new_model.predict(x_for_pred.reshape(days,TIME_STEPS,len(train_cols)), batch_size = days)
    pred_org = (new_pred * sc.data_range_[0]) + sc.data_min_[0]
    if days == 1:
        print("The price prediction for tomorrow is {:.3f}".format(pred_org[0][0]))
    else:
        print("The price predictions for the following {:.0f} days are {}".format(days, pred_org))
    return pred_org
